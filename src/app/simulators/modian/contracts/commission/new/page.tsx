...
const CONTRACTS_STORAGE_KEY = 'modian.contracting.contracts';
export default function ModianContractsCommissionNewPage() {
  const router = useRouter();
  const [currentStep, setCurrentStep] = React.useState(0);
  const [role, setRole] = React.useState<Role>('employer');
  const counterpartyLabel = role === 'employer' ? 'حق العملکار' : 'آمر';
  const selfRoleLabel = role === 'employer' ? 'آمر' : 'حق العملکار';
  ...
  const [employerType, _setEmployerType] = React.useState('');
  ...
  const [commissionFeeRate, setCommissionFeeRate] = React.useState('');
  ...
  const handleCancelConfirm = () => {
    ...
    router.push('/simulators/modian/contracts/commission');
  };
  ...
  const handleCancelClose = () => {
    ...
    if (currentStep === STEPS.length - 1) {
      const nextRow = {
        ...
        commissionFeeRate: commissionFeeRate || '—',
        contractDate:
          contractDateDisplay ||
      ...
      try {
        ...
        localStorage.setItem(CONTRACTS_STORAGE_KEY, JSON.stringify(next));
      } catch {
        // silent
      }
  ...
  const renderStepContent = () => {
    ...
    if (currentStep === 2) {
      ...
      if (personType === 'حقیقی غیر ایرانی') {
        ...
        identityLabel = `شناسه ملی ${counterpartyLabel} وارد شده:`;
    ...
    if (currentStep === 3) {
      ...
      role === 'employer' ? 'نام حق العملکار:' : 'نام آمر:';
      ...
      return (
        ...
        <div className="w-full max-w-[420px] space-y-6">
          ...
          <label className="relative block">
                ...
                <select
                  className={[
                    'peer w-full appearance-none rounded-md border bg-white px-4 py-3 text-right text-sm md:text-base text-gray-800 outline-none focus:ring-0',
                    errors.contractType
                      ? 'border-red-500 focus:border-red-600'
                      : 'border-gray-300 focus:border-green-700',
                  ].join(' ')}
                ...
                >
                  ...
                  <option value="قرارداد حق العملکاری خرید">
                    قرارداد حق العملکاری خرید
                  </option>
                  <option value="قرارداد حق العملکاری فروش">
                    قرارداد حق العملکاری فروش
                  </option>
                </select>
              </label>
              {errors.contractType && (
                <div className="mt-2 flex items-center gap-1 text-xs text-red-600">
                  ...
                  >
                    <path
                      d="M12 3L3 21h18L12 3zm0 6a1 1 0 ... 0v-5a1 1 0 011-1zm0 9a1.25 1.25 0 110-2.5 1.25 1.25 0 010 2.5z"
                      fill="currentColor"
                    />
                  </svg>
                  <span>
                    {contractTypeErrorMessage || 'پرکردن فیلد اجباری است.'}
                  </span>
                </div>
              )}
            ...
            <div className="mt-6 w-full max-w-[420px]">
              ...
              >
                استعلام قرارداد های موجود در تاریخ وارد شده
              </button>
            </div>
      ...
      return (
        <div className="mt-4 flex flex-col items-center">
          ...
          <div className="w-full max-w-[360px] space-y-6">
            ...
            {/* نرخ کارمزد حق العملکاری: (اختیاری) */}
            <label className="relative block">
              <span className="pointer-events-none absolute right-4 -top-2 bg-white px-1 text-xs text-gray-700">
                نرخ کارمزد حق العملکاری: (اختیاری)
              </span>
              <input
                ...
                onChange={(e) => setCommissionFeeRate(e.target.value)}
              />
            </label>
          ...
          <div className="w-full max-w-[920px] space-y-8">
            ...
            <div className="rounded-md border border-gray-200 bg-white px-6 py-6">
              ...
              <div className="space-y-4 text-sm text-gray-800">
                ...
                <div className="flex items-center justify-between border-b border-dashed border-gray-300 pb-3">
                  <span className="text-gray-700">نرخ کارمزد حق العملکاری :</span>
                  <span className="min-w-[120px] text-left">
                    {commissionFeeRate || '—'}
                  </span>
                </div>
    ...
    <div className="mx-auto w-full max-w-[1200px] px-4 py-6">
          ...
          <div className="w-full max-w-[720px] rounded-md border border-blue-300 bg-blue-50 p-6 text-right shadow-lg">
            ...
            {monjiContext === 'economic' && (
              <>
                ...
                در حالت واقعی اطلاعات آمر/حق العملکار در سامانه ثبت نام الکترونیکی سازمان امور مالیاتی کنترل میشود
                </p>
                <p className="mb-1 text-sm text-gray-800">
      ...
      {showIdentityResultModal && (
        <div className="fixed inset-0 z-40 flex items-start justify-center bg-black/40 lg:pr-[280px]">
          ...
          >
            <p className="mb-6 text-sm text-gray-800 text-right">
              کاربر گرامی، اطلاعات {counterpartyLabel} یافت نشد. به منظور ثبت قرارداد باید
              با انتخاب گزینه «تایید» سایر اطلاعات {counterpartyLabel} را وارد کنید.
            </p>
            <div className="flex justify-end gap-6">
              <button
                type="button"
