...
export default function LoginForm() {
  ...
  const handleLogin = async () => {
    if (!username) return setError("شماره تلفن همراه وارد نشده است");
    if (!password) return setError("رمز عبور وارد نشده است");
    if (!captchaAnswer) return setError("کد امنیتی وارد نشده است");
  ...
  return (
    <div className="w-full max-w-sm sm:max-w-md lg:max-w-[420px] flex justify-center items-start mx-auto">
      <Card className="relative w-full bg-[url('/images/input-page.png')] bg-cover bg-no-repeat bg-center border border-gray-200 rounded-tl-none rounded-br-none rounded-tr-xl rounded-bl-xl shadow-xl min-h-[600px] flex flex-col justify-start pt-8 overflow-visible">
        <CardContent className="relative flex flex-col justify-start items-center h-full px-4 sm:px-6 pt-6 sm:pt-10 pb-6 mt-6 sm:mt-10">

          <div className="w-full flex flex-col items-center space-y-6">
            {/* شماره تلفن همراه */}
            <div className="relative w-full max-w-[260px] sm:max-w-[200px] mx-auto">
              <input
                type="text"
                placeholder="شماره تلفن همراه"
                value={username}
                onChange={(e) => setUsername(e.target.value)}
                className="text-right text-base sm:text-sm h-10 sm:h-9 w-full pr-8 border-b border-gray-400 rounded-none placeholder:text-gray-500 focus:outline-none"
              />
            ...
            {/* کپچا */}
            <div className="relative w-full max-w-[260px] sm:max-w-[200px] mx-auto">
              <div className="flex items-center justify-center flex-wrap text-sm text-gray-800 mb-1 flex-row-reverse gap-6 sm:gap-10">
                <img
          ...
          <div
            onClick={handleLogin}
            className="w-full max-w-[260px] h-12 sm:h-14 mt-8 flex items-center justify-between flex-row-reverse ml-auto self-start rounded-full border border-gray-300 bg-white px-4 cursor-pointer hover:bg-gray-50 transition text-base sm:text-sm"
          >
